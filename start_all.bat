...
echo [1/4] Configuration des ports...
...
echo [2/4] Démarrage du Backend (port 3006)...
start "NovoRise — Backend :3006" cmd /k "cd /d "%~dp0backend" && set PORT=3006 && npm run dev"
timeout /t 3 /nobreak >nul

:: ─── Lancement Frontend Public sur port 3005 ─────────────────────────────────
echo [3/4] Démarrage du Frontend Public (port 3005)...
start "NovoRise — Frontend :3005" cmd /k "cd /d "%~dp0" && set VITE_PORT=3005 && npx vite --port 3005"
timeout /t 3 /nobreak >nul

:: ─── Lancement Portail Admin sur port 3007 ────────────────────────────────────
echo [4/4] Démarrage du Portail Admin (port 3007)...
start "NovoRise — Admin Portal :3007" cmd /k "cd /d "%~dp0" && npx vite --port 3007 admin.html"
timeout /t 3 /nobreak >nul

:: ─── Ouverture des navigateurs ───────────────────────────────────────────────
echo.
echo  ✅ Services tous lancés !
echo  📌 App Publique : http://localhost:3005
echo  🔐 Portail Admin : http://localhost:3007
echo.
